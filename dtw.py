...
def getCostMatrix(xs, ys, costFn):
    ...
    costMat = np.zeros((len(xs), len(ys)))  # np.array([ [ costFn(x, y) for y in ys ] for x in xs ])
    assert np.shape(costMat) == (len(xs), len(ys))
    return costMat


def getCumCostMatrix(xs, ys, max_window=10, costFn=lambda x, y: abs(x - y)):
    xSize, ySize = len(xs), len(ys)

    cumMat = np.zeros((xSize + 1, ySize + 1))
    cumMat[:, :] = float('inf')
    cumMat[0, 0] = 0.0
    for i in range(1, xSize):
        cumMat[i, 0] = cumMat[i - 1, 0] + costFn(i, 0)

    for j in range(1, ySize):
        cumMat[0, j] = cumMat[0, j - 1] + costFn(0, j)

    for i in range(xSize):
        for j in range(max(1, i - max_window),
                       min(ySize, i + max_window)):
            cumMat[i + 1, j + 1] = min(
                cumMat[i, j],
                cumMat[i, j + 1],
                cumMat[i + 1, j]
            )
            cumMat[i + 1, j + 1] += costFn(xs[i], ys[j])  # costMat[i, j]
...
def dtw(xs, ys, costFn):
    ...
    """
    cumMat = getCumCostMatrix(xs, ys, costFn=costFn)
